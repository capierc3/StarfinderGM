...
public class ShipYard {
    ...
    /**
     * constructor that fills the part arrays by reading the text file of
     * information and parsing it into parts.
     * Uses the fillYard method after setting the text file.
     */
    public ShipYard() {
        try {
    ...
    public ArrayList<PowerCore> getCoresBySize(String size) {
        ...
        for (PowerCore powerCore : powerCores) {
            if (powerCore.getSize().contains(Character.toString(sizeC))) {
                parts.add(powerCore);
            }
        }
    ...
    public ArrayList<ShipWeapon> getWeaponsByType(String type) {
        ...
        for (ShipWeapon weapon : weapons) {
            if (weapon.getType().contains(type)) {
                weaponType.add(weapon);
            }
        }
        return weaponType;
    }
    ...
    private void inExpansion() {
        ...
        while (!line.equalsIgnoreCase("")) {
            ...
            for (String s : desc) {
                ...
                if (name.equalsIgnoreCase(s.split("<>")[0])) {
                    bay.setDescription(s.split("<>")[1]);
                }
            }
            bays.add(bay);
            nextLine();
