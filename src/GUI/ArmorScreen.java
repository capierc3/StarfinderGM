...
public class ArmorScreen {

    private static EquipmentStore equipmentStore = new EquipmentStore(0,20,"All", "Armor",EquipmentStore.size.All);
    ...
    public static void Run(BorderPane root){
        ...
        root.setLeft(left(root, equipmentStore.getArmorNames()));
        root.setRight(null);
    ...
    private static BorderPane center(BorderPane root,String name){
        ...
        type.getItems().addAll(Armor.getTypes());
        ...
        type.setOnAction(actionEvent -> {
            equipmentStore = new EquipmentStore(Integer.parseInt(levelLow.getValue()),Integer.parseInt(levelHigh.getValue()),type.getValue(),"Armor", EquipmentStore.size.All);
            try {
                equipmentStore.fillArmor();
            } catch (SQLException e) {
                e.printStackTrace();
        ...
        equipmentStore.getEquipment(name,armor,armor.tableName,armor.getKeys());
        if (armor.getName()!=null){
            pane.setCenter(SFText.create(armor.toString()));
        } else pane.setCenter(SFText.create("Armor"));
        return pane;
    }
}
